...
function songlist_getUserSearchForm($action = false)
{
    ...
    $songlistHandler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Profile', 'objects');
    // Get fields
    ...
    $catHandler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Category');
...
{
    if (!is_object($object)) {
        $handler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Voice');
        $object  = $handler->create();
    }
...
function songlist_albums_get_form($object, $as_array = false)
{
    ...
    $ele['cid'] = new \XoopsModules\Songlist\Form\SelectCategoryForm((false === $as_array ? _FRM_SONGLIST_FORM_ALBUMS_CATEGORY : ''), $id . '[cid]', $object->getVar('cid'), 1, false, false, false);
...
function songlist_artists_get_form($object, $as_array = false)
{
    ...
    $ele['cids'] = new \XoopsModules\Songlist\Form\SelectCategoryForm((false === $as_array ? _FRM_SONGLIST_FORM_ARTISTS_CATEGORY : ''), $id . '[cids]', $object->getVar('cids'), 7, true, false, false);
...
{
    if (!is_object($object)) {
        $handler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Category');
        $object  = $handler->create();
    }
    ...
    $ele['pid'] = new \XoopsModules\Songlist\Form\SelectCategoryForm((false === $as_array ? _FRM_SONGLIST_FORM_CATEGORY_PARENT : ''), $id . '[pid]', $object->getVar('pid'), 1, false, $object->getVar('cid'));
...
{
    if (!is_object($object)) {
        $handler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Utf8map');
        $object  = $handler->create();
    }
...
{
    if (!is_object($object)) {
        $handler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Requests');
        $object  = $handler->create();
    }
...
function songlist_songs_get_form($object, $as_array = false)
{
    ...
    $ele['cid'] = new \XoopsModules\Songlist\Form\SelectCategoryForm((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_CATEGORY : ''), $id . '[cid]', (isset($_REQUEST['cid']) ? $_REQUEST['cid'] : $object->getVar('cid')), 1, false);
    $ele['cid']->setDescription((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_CATEGORY_DESC : ''));
    if ($GLOBALS['songlistModuleConfig']['genre']) {
        $ele['gids'] = new \XoopsModules\Songlist\Form\SelectGenreForm((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_GENRE : ''), $id . '[gids]', (isset($_REQUEST['gids']) ? $_REQUEST['gids'] : $object->getVar('gids')), 8, true);
        $ele['gids']->setDescription((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_GENRE_DESC : ''));
    }
    if ($GLOBALS['songlistModuleConfig']['voice']) {
        $ele['vcid'] = new \XoopsModules\Songlist\Form\SelectVoiceForm((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_VOICE : ''), $id . '[vcid]', (isset($_REQUEST['vcid']) ? $_REQUEST['vcid'] : $object->getVar('vcid')), 1, false);
        $ele['vcid']->setDescription((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_VOICE_DESC : ''));
    }
    if ($GLOBALS['songlistModuleConfig']['album']) {
        $ele['abid'] = new \XoopsModules\Songlist\Form\SelectAlbumForm((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_ALBUM : ''), $id . '[abid]', $object->getVar('abid'), 1, false);
        $ele['abid']->setDescription((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_ALBUM_DESC : ''));
    }
    $ele['aids'] = new \XoopsModules\Songlist\Form\SelectArtistForm((false === $as_array ? _FRM_SONGLIST_FORM_SONGS_ARTISTS : ''), $id . '[aids]', $object->getVar('aids'), 7, true);
...
{
    if (!is_object($object)) {
        $handler = \XoopsModules\Songlist\Helper::getInstance()->getHandler('Votes');
        $object  = $handler->create();
    }
